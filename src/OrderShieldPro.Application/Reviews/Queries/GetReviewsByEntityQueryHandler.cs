...
public class GetReviewsByEntityQueryHandler : IRequestHandler<GetReviewsByEntityQuery, PaginatedList<ReviewDto>>
{
    private readonly IReviewRepository _repository;
    private readonly IIdentityService _identityService;

    public GetReviewsByEntityQueryHandler(IReviewRepository repository, IIdentityService identityService)
    {
        _repository = repository;
        _identityService = identityService;
    }

    public async Task<PaginatedList<ReviewDto>> Handle(GetReviewsByEntityQuery request, CancellationToken cancellationToken)
    {
        var (items, totalCount) = await _repository.GetByEntityIdAsync(
            request.TradeEntityId, request.Page, request.PageSize, cancellationToken);
